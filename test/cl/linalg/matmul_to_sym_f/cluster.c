...
int do_bench(rt_perf_t* perf, int events) {
    ...
    linalg_matmul_to_sym_f(a_stm_l1, b_stm_l1, N_DIM, y_acq_l1);
...
void cluster_entry(void* arg) {
    ...
    b_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * N_DIM * N_DIM);
    y_acq_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * N_DIM * N_DIM);
