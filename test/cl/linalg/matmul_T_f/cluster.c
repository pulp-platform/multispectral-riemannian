...
int do_bench(rt_perf_t* perf, int events) {
    ...
    linalg_matmul_T_f(a_stm_l1, b_stm_l1, M_DIM, N_DIM, O_DIM, y_acq_l1);
    ...
    for (int _m = 0; _m < M_DIM; _m++) {
        for (int _o = 0; _o < O_DIM; _o++) {
            int _idx = _m * O_DIM + _o;
...
void cluster_entry(void* arg) {
    ...
    // allocate memory
    a_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * N_DIM);
    b_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * O_DIM * N_DIM);
    y_acq_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * O_DIM);
    y_exp_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * O_DIM);
