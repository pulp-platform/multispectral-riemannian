...
int do_bench(rt_perf_t* perf, int events) {
    ...
    linalg_matsub_f(a_stm_l1, b_stm_l1, M_DIM, N_DIM, y_acq_l1);
...
void cluster_entry(void* arg) {
    ...
    b_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * N_DIM);
    y_acq_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * N_DIM);
    y_exp_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM * N_DIM);
