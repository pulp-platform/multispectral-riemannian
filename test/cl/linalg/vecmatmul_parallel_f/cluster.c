...
void bench_kernel(void* args) {
    ...
    linalg_vecmatmul_parallel_f(core_id, NUM_WORKERS, a_stm_l1, b_stm_l1, M_DIM, N_DIM, y_acq_l1);
}
...
int do_bench(rt_perf_t* perf, int events) {
    ...
    for (int _n = 0; _n < N_DIM; _n++) {
        float _rel_diff = rel_diff(y_exp_l1[_n], y_acq_l1[_n]);
        max_rel_diff = insn_fmax(max_rel_diff, _rel_diff);
        if (_rel_diff > EPSILON) {
            printf("error at N=%d: acq=%.2e, exp=%.2e\n", _n, y_acq_l1[_n], y_exp_l1[_n]);
            error = 1;
        }
    }
...
void cluster_entry(void* arg) {
    ...
    // allocate memory
    a_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * M_DIM);
    b_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * N_DIM * M_DIM);
    y_acq_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * N_DIM);
    y_exp_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(float) * N_DIM);
