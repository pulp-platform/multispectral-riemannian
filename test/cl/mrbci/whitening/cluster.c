...
RT_CL_DATA static int16_t* x_stm_l1;
RT_CL_DATA static int32_t* y_acq_l1;
RT_CL_DATA static int32_t* workspace_l1;
...
int do_bench(rt_perf_t* perf, int events) {
    ...
    mrbci_whitening(x_stm_l1, FREQ_IDX, y_acq_l1, workspace_l1);
...
void cluster_entry(void* arg) {
    ...
    // allocate memory
    x_stm_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int16_t) * MRBCI_C * MRBCI_C);
    y_acq_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int32_t) * MRBCI_C * MRBCI_C);
    workspace_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int32_t) * MRBCI_C * MRBCI_C);
