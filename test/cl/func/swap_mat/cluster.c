...
RT_CL_DATA static uint32_t* a_exp_vec_l1;
...
int do_bench(rt_perf_t* perf, int events) {
    ...
    func_swap_mat(a_vec_l1, b_vec_l1, N_DIM, M_DIM, STRIDE_A, STRIDE_B);

    rt_perf_stop(perf);

    int error = 0;
    for (int i = 0; i < N_DIM; i++) {
        for (int j = 0; j < STRIDE_A; j++) {
            if (a_vec_l1[i * STRIDE_A + j] != a_exp_vec_l1[i * STRIDE_A + j]) {
                error = 1;
                printf("error of A at: %d, %d: acq=%u, exp=%u\n", i, j, a_vec_l1[i * STRIDE_A + j], a_exp_vec_l1[i * STRIDE_A + j]);
            }
        }
    }
    for (int i = 0; i < N_DIM; i++) {
...
void cluster_entry(void* arg) {
    ...
    a_exp_vec_l1 = rt_alloc(RT_ALLOC_CL_DATA, sizeof(uint32_t) * N_DIM * STRIDE_A);
    ...
    rt_dma_memcpy((unsigned int)a_exp_vec, (unsigned int)a_exp_vec_l1, sizeof(a_exp_vec), RT_DMA_DIR_EXT2LOC, 1, &copy);
